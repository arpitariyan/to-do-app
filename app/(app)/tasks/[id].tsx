...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, Switch, Platform, KeyboardAvoidingView, ActivityIndicator, Alert, Linking, TextInput } from 'react-native';
...
export default function EditTaskScreen() {
  ...
  const handleOpenAttachment = (url: string) => {
    Linking.openURL(url).catch(err => alert('Could not open file'));
  };
            ...
            {attachments.length > 0 && (
              <View style={styles.attachmentList}>
                {attachments.map(file => (
                  <TouchableOpacity key={file.id} style={[styles.attachmentItem, { backgroundColor: colors.bg3, borderColor: colors.border }]} onPress={() => file.url && handleOpenAttachment(file.url)}>
               ...
               <View style={{ flex: 1 }}>
                 {subtasks.map(st => (
                   <View key={st.id} style={[styles.subtaskItem, { borderBottomColor: colors.border }]}>
                     <Ionicons name={st.completed ? "checkmark-circle" : "ellipse-outline"} size={16} color={st.completed ? colors.success : colors.textMuted} />
          ...
          >
            {updateTask.isPending ? (
              ...
              <>
                ...
                <Text style={[textStyles.bodyLg, { color: '#fff', fontWeight: '600', marginLeft: spacing.sm }]}>Save Changes</Text>
              </>
            )}
          </TouchableOpacity>
        </View>
