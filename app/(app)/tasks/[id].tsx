...
import { scheduleTaskReminder, cancelTaskReminder } from '../../../src/hooks/useNotifications';
...
export default function EditTaskScreen() {
  ...
  const { control, handleSubmit, formState: { errors }, watch, setValue, reset } = useForm<TaskFormData>({
  ...
  useEffect(() => {
    if (task) {
      reset({
        ...
        pinned: task.pinned,
        repeatType: (task.repeatType as RepeatType) || 'none',
      });
      ...
      if (task.reminders && task.reminders.length > 0) {
        setReminderType(task.reminders[0]);
      }
      ...
      if (task.subtasks) {
        setSubtasks(task.subtasks.map(st => {
          ...
          } catch(e) { return { id: Date.now().toString(), title: st, completed: false }; }
        }));
      }

      if (task.attachments) {
        setAttachments(task.attachments.map(a => {
          try {
    ...
    updateTask.mutate(
      {
        ...
        payload: {
          ...
          tags: task?.tags || [], // preserve original tags
        ...
        onSuccess: async () => {
          ...
          if (finalDueAt && reminderType !== 'none') {
            ...
            if (triggerTime.getTime() > Date.now()) {
              await scheduleTaskReminder(id, data.title, data.description || '', triggerTime, data.repeatType as any);
            }
          }
          router.back();
        },
    ...
    return (
      <Modal visible={showDeleteModal} transparent animationType="slide" onRequestClose={() => setShowDeleteModal(false)}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: colors.bg1 }]}>
            <View style={styles.modalHeader}>
              <View style={[styles.modalIconBox, { backgroundColor: errorSoft }]}>
                <Ionicons name="trash" size={24} color={colors.error} />
              </View>
              <Text style={[textStyles.headingMd, { color: colors.textPrimary, marginLeft: spacing.sm }]}>Delete Task?</Text>
            </View>
            <Text style={[textStyles.bodyMd, { color: colors.textSecondary, marginBottom: spacing.xl }]}>
              Are you sure you want to delete this task? This action cannot be undone.
            </Text>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              <TouchableOpacity style={[styles.modalBtn, { backgroundColor: colors.bg2, flex: 1, marginRight: 8 }]} onPress={() => setShowDeleteModal(false)}>
                <Text style={[textStyles.bodyMd, { color: colors.textPrimary, fontWeight: '600' }]}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.modalBtn, { backgroundColor: colors.error, flex: 1, marginLeft: 8 }]} onPress={() => {
                cancelTaskReminder(id);
                deleteTask.mutate(id, { onSuccess: () => router.back() });
              }}>
                {deleteTask.isPending ? <ActivityIndicator color="#FFF" /> : <Text style={[textStyles.bodyMd, { color: '#FFF', fontWeight: '700' }]}>Delete</Text>}
              </TouchableOpacity>
            </View>
          </View>
        </View>
  ...
  return (
    <Screen noPadding>
      <View style={[styles.header, { paddingTop: insets.top + spacing.md }]}>
        ...
        <Text style={[textStyles.headingMd, { color: colors.textPrimary }]}>Edit Task</Text>
        <TouchableOpacity onPress={() => setShowDeleteModal(true)} style={styles.iconCircleBtn}>
          ...
          <View style={[styles.subtaskContainer, { backgroundColor: isDark ? colors.bg2 : colors.bg1, borderColor: colors.border }]}>
             {subtasks.map(st => (
               <View key={st.id} style={[styles.subtaskRow, { borderBottomColor: colors.border }]}>
                 <TouchableOpacity 
                   style={[styles.circleCheck, { borderColor: st.completed ? colors.success : colors.textMuted, backgroundColor: st.completed ? colors.success : 'transparent', justifyContent: 'center', alignItems: 'center' }]}
                   onPress={() => setSubtasks(subtasks.map(s => s.id === st.id ? { ...s, completed: !s.completed } : s))}
                 >
                   {st.completed && <Ionicons name="checkmark" size={16} color="#FFF" />}
                 </TouchableOpacity>
                 <Text style={[textStyles.bodyMd, { color: colors.textPrimary, flex: 1, marginLeft: spacing.sm, textDecorationLine: st.completed ? 'line-through' : 'none' }]} numberOfLines={1}>{st.title}</Text>
      ...
      <View style={[styles.floatingAction, { bottom: Math.max(insets.bottom, spacing.base) + spacing.md }]}>
        <TouchableOpacity 
          style={[styles.saveBtn, { backgroundColor: updateTask.isPending || isUploading ? colors.textMuted : colors.accent }]}
        ...
        >
          {updateTask.isPending ? (
            ...
            <>
              ...
              <Text style={[textStyles.bodyLg, { color: '#fff', fontWeight: '700', fontSize: 18 }]}>Save Changes</Text>
            </>
          )}
        </TouchableOpacity>
      </View>
